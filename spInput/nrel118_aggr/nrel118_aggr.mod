...
table bus_data IN "CSV" "../../energySystems/nrel118_aggr/busData.csv" : 
...
table lines_data IN "CSV" "../../energySystems/nrel118_aggr/lineData.csv" : 
...
table gen_data IN "CSV" "../../energySystems/nrel118_aggr/genData.csv" : 
...
table renew_data IN "CSV" "../../energySystems/nrel118_aggr/renewData.csv" : 
...
table load_data IN "CSV" "../../energySystems/nrel118_aggr/loadData.csv" : 
LOAD <- [LOAD], loadBus, loadRegion, Pd, Qd;

## Fixed parameters: initial generation, deterministic demand, and renew generation
param gInit{g in GEN} := (0.1*genMin[g]+0.9*genMax[g]);
     
set D, dimen 2;
param Period; param Region; param  realLoad{D}; 
table realLoad_data IN "CSV" "../../energySystems/nrel118_aggr/realLoad_glpk.csv" :
D <- [Period,Region], realLoad;

set W, dimen 2;
param renewPower{W};
table renewPower_data IN "CSV" "../../energySystems/nrel118_aggr/renewPower_glpk.csv" :
...
# var p1{n in TIME, l in LINES};
# var th1{n in TIME, b in BUS};
var p1{n in TIME, l in LINES}  <= 2*flowMax[l], >= 2*flowMin[l];
